...
class UI(pm.uitypes.Window):
    ...
    def __init__(self):
        ...
        with form:
            ...
            pm.menuItem(
                l="UI: Toggle tablayout", c=pm.Callback(self.ui_toggleTablayout)
            )
    ...
    def ui_makeDockable(self):
        ...
        if not self.tryConfirmDialog("Make UI dockable?\n", 1):
            return
        # create dockControl
        self.dockControl = pm.dockControl(
            self._DOCKCONTROL,
            l=self,
            content=self,
            area="left",
    ...
    def set_sort(self):
        ...
        for each in self.sets:
            if each.tabIndex in dic:
                dic[each.tabIndex][
                    each.referencePrefix + each.tabName + each.shortName()
                ] = each
            else:
                dic[each.tabIndex] = {
                    each.referencePrefix + each.tabName + each.shortName(): each
                }
        ordered = []
        for eachIndexKey in sorted(dic.keys()):
            for eachTabKey in sorted(dic[eachIndexKey].keys()):
    ...
    def set_newElement(self, elementType):
        ...
        self.set_getActive(
            "Can't add %s to referenced tab!" % elementType
        ).element_create(elementType)
...
class Set(pm.uitypes.FormLayout):  # scroll layout?
    ...
    def __init__(self, name, parentUi, fromSet=None):
        ...
        pm.menuItem(
            label="New selection",
            c=pm.Callback(self.parentUi.set_newElement, Selection._TYPE),
        )
        pm.menuItem(
            label="New pose", c=pm.Callback(self.parentUi.set_newElement, Pose._TYPE)
        )
        ...
        for (
            ...
            self.set.dnSetMembers.inputs()
        ):  # pm.sets( self.set, q=1 ):# for propper order
    ...
    def delete(self, forced=0, uiOnly=0):
        ...
        # pop-up
        if not forced:
            if not self.parentUi.tryConfirmDialog(
                'Delete tab: "%s" ?' % self.getUiName()
            ):
                return
    ...
    def setPosition(self):
        ...
        if insertAfterIndex == 0:
            ...
            insertAfter = allMenuItems[
                insertAfterIndex - 1
            ]  # .getItemListLong() # maya(pymel) 2012+
        ...
        if not insertAfter:
            if allMenuItems:
                self.asMenuItem = pm.menuItem(
                    uiName, label=uiName, p=self.parentUi.setsOptionMenu, ia=""
                )
            else:
                self.asMenuItem = pm.menuItem(
                    uiName, label=uiName, p=self.parentUi.setsOptionMenu
                )
        else:
            """
    ...
    def element_orderUi(self):
        ...
        newSetOrder = (
            self.set.dnSetMembers.inputs()
        )  # pm.sets( self.set, q=1 ) not correct order# self.set.members() / asSelectionSet # MPlug error..
        newElementList = []
...
class SetElement(pm.uitypes.FormLayout):
    ...
    def getName(elementType, fromSet=None):
        ...
        # get name from user
        result = pm.promptDialog(
            title=("New %s" % elementType),
            message=("Name for new %s:" % elementType),
            button=["OK", "Cancel"],
            defaultButton="OK",
            cancelButton="Cancel",
            dismissString="Cancel",
    ...
    def __init__(self, name, parentUi, fromSet=None):
        ...
        if fromSet:
            ...
            self.set.addAttr(
                self._ATTR_NAME, dt="string"
            )  # self.set.setAttr( self._ATTR_NAME, self.name, force=1 ) # problem with some signs (umlaute),..
        ...
        pm.formLayout(
            self.parentUi, e=1, attachForm=[(self, "left", 0), (self, "right", 0)]
        )
        ...
        with self:
            ...
            pm.menuItem(
                l="New %s" % Selection._TYPE,
                c=pm.Callback(self.parentUi.element_create, Selection._TYPE),
                en=dynamicSel,
            )
            pm.menuItem(
                l="New %s" % Pose._TYPE,
                c=pm.Callback(self.parentUi.element_create, Pose._TYPE),
                en=dynamicSel,
            )
            pm.menuItem(
                l="Delete %s" % self._TYPE, c=pm.Callback(self.delete), en=dynamicSel
            )
            pm.menuItem(divider=1)
    ...
    def cmd_rename(self):
        ...
        # get name from user
        result = pm.promptDialog(
            title=("Rename %s" % self._TYPE),
            message='Rename %s: "%s"' % (self._TYPE, self.name),
            text=self.name,
            button=["OK", "Cancel"],
            defaultButton="OK",
            cancelButton="Cancel",
    ...
    def cmd_changePosition(self):
        ...
        if result == "OK":
            ...
            #     get sets
            allSets = (
                self.parentUi.set.dnSetMembers.inputs()
            )  # pm.sets( self.parentUi.set, q=1 ) # self.parentUi.set.members() # .members() error in maya 2011
    ...
    def object_getSceneName(self, objectName):
        ...
        return self.parentUi.referencePrefix + objectName.replace(
            "|", "|" + self.parentUi.referencePrefix
        )
...
class Pose(SetElement):
    ...
    def __init__(self, name, parentUi, fromSet=None):
        ...
        for each in self.set.attr(self._ATTR_POSEVALUES).get():
            ...
            if eachNode not in self.poseValues:
        ...
        with self.middleClickMenu:
            ...
            pm.menuItem(
                l="Pose selected - XY%", c=pm.Callback(self.ui_poseCustomPercent, True)
            )
            pm.menuItem(divider=1)
    ...
    def ui_poseCustomPercent(self, poseSelected=False):
        ...
        with form:
            ...
            pm.button(
                l="Apply",
                c=pm.Callback(
                    self.ui_poseCustomPercentCmd, igPosePercent, poseSelected
                ),
            )
    ...
    def cmd_selectNodes(self):
        ...
        pm.select(list(self.poseValues.keys()))
...
class Selection(SetElement):
    ...
    def __init__(self, name, parentUi, fromSet=None):
        ...
        with self.rightClickMenu:
            pm.menuItem(divider=1)
            pm.menuItem(
                l="Add selected", en=dynamicSel, c=pm.Callback(self.member_addSelected)
            )
            pm.menuItem(
                l="Remove selected",
                en=dynamicSel,
                c=pm.Callback(self.member_removeSelected),
            )
            pm.menuItem(
                l="Remove disabled",
                en=dynamicSel,
                c=pm.Callback(self.member_removeDisabled),
            )
            pm.menuItem(divider=1)
        ...
        with horizontal:
            ...
            self.button = pm.button(
                l=self.getUiName(), h=1, w=1, c=pm.Callback(self.cmd_select, "only")
            )
        ...
        if cbAttrs:
            ...
            for eachAttr in pm.channelBox("mainChannelBox", q=1, sma=1):
                for eachObj in objects:
                    ...
                    if pm.getAttr(eachAttrFull, k=1) and not pm.getAttr(
                        eachAttrFull, lock=1
                    ):
    ...
    def cmd_resetAttributes(self):
        ...
        for eachAttr in self.cmd_getKeyableAttrs():
            ...
            defaultValue = pm.attributeQuery(
                eachAttrStr[eachAttr.find(".") + 1 :],
                n=eachAttrStr[: eachAttrStr.find(".")],
                listDefault=1,
            )[0]
...
class MemberWindow(pm.uitypes.Window):
    ...
    def __new__(cls, parentUi):
        ...
        if pm.window(cls._windowName, exists=True):
            if not parentUi.parentUi.parentUi.tryConfirmDialog(
                'Close existing "' + cls._windowName + '"?'
            ):
                return
    ...
    def __init__(self, parentUi):
        ...
        with formRoot:
            ...
            with formMembers:
                # member list
                self.memberTSL = pm.textScrollList(
                    allowMultiSelection=1, sc=pm.Callback(self.cmd_TSL_selection)
                )
                # UP / DOWN buttons
                ...
                with formUpDown:
                    pm.button(
                        ebg=0, l="UP", c=pm.Callback(self.cmd_UP)
                    )  # , nbg=0)# maya 2013+
    ...
    def cmd_close(self):
        ...
        if not self.unsavedChanges:
            ...
            return
        if self.parentUi.parentUi.parentUi.tryConfirmDialog(
            "Close and lose unsaved changes?"
        ):
            pm.deleteUI(self._windowName)
