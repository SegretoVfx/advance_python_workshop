...
def warning(title, message, parent=None):
    return QtWidgets.QMessageBox.warning(
        parent, title, message, QtWidgets.QMessageBox.Ok, QtWidgets.QMessageBox.Ok
    )
...
class CommandButtonDialog(QtWidgets.QDialog):
    ...
    def change_state(self, *_):
        save_optionvar(LAST_COMMAND_LANGUAGE, self.language_buttons.checkedId())
...
class SettingsPaster(QtWidgets.QDialog):
    ...
    def settings(self):
        return [
            ...
            if cb.isChecked() and self.groups[category].isChecked()
        ]
...
class UpdateAvailableDialog(QtWidgets.QDialog):
    def __init__(self, version, parent=None):
        ...
        self.check_cb.stateChanged.connect(self.change_check_for_update_preference)
