...
from .geometry import Transform, get_combined_rects
from .painting import draw_editor, draw_shape
...
class ShapeEditArea(QtWidgets.QWidget):
    ...
    def list_shapes(self):
        if self.lock_background_shape:
            return [shape for shape in self.shapes if not shape.is_background()]
        return self.shapes
    ...
    def mousePressEvent(self, event):
        ...
        conditions = (
            hovered_shape
            and hovered_shape not in self.selection
            and not self.transform.direction
        )
    ...
    def key_event(self, event, pressed):
        ...
        self.selection.mode = get_selection_mode(
            shift=self.shit_pressed, ctrl=self.ctrl_pressed
        )
