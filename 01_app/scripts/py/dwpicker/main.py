...
from py.dwpicker import designer
from .dialog import (
    warning,
    question,
    get_image_path,
...
from .optionvar import (
    ...
    LAST_SAVE_DIRECTORY,
    NAMESPACE_TOOLBAR,
    USE_ICON_FOR_UNSAVED_TAB,
    WARN_ON_TAB_CLOSED,
    save_optionvar,
    append_recent_filename,
    save_opened_filenames,
)
from .picker import PickerView, list_targets
...
from .scenedata import (
    load_local_picker_data,
    store_local_picker_data,
    clean_stray_picker_holder_nodes,
)
from .templates import BUTTON, PICKER, BACKGROUND
...
""".format(
    version=".".join(str(n) for n in VERSION), release=RELEASE_DATE
)
...
class DwPicker(DockableBase, QtWidgets.QWidget):
    ...
    def dockCloseEventTriggered(self):
        ...
        result = QtWidgets.QMessageBox.question(
            ...
            "Save ?",
            msg,
            buttons=(QtWidgets.QMessageBox.SaveAll | QtWidgets.QMessageBox.Close),
    ...
    def register_callbacks(self):
        self.unregister_callbacks()
        callbacks = {
            om.MSceneMessage.kBeforeNew: [self.close_tabs, self.update_namespaces],
    ...
    def save_tab(self, index):
        msg = "Picker contain unsaved modification !\n" "Woud you like to continue ?"
        result = QtWidgets.QMessageBox.question(
            None,
            "Save ?",
            msg,
    ...
    def close_tab(self, index, force=False, store=False):
        if self.modified_states[index] and force is False:
            if not self.save_tab(index):
                return
        elif cmds.optionVar(query=WARN_ON_TAB_CLOSED) and not question(
            "Warning", CLOSE_TAB_WARNING
        ):
            return
    ...
    def call_new(self):
        self.add_picker({"general": PICKER.copy(), "shapes": []})
        self.store_local_pickers_data()
    ...
    def call_edit(self):
        ...
        if self.editors[index] is None:
            ...
            undo_manager = self.undo_managers[index]
            editor = designer.editor.PickerEditor(
                picker_data=data, undo_manager=undo_manager, parent=self
            )
            picker = self.pickers[index]
