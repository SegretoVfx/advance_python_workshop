...
from .geometry import split_line, get_combined_rects
from .optionvar import SYNCHRONYZE_SELECTION, ZOOM_BUTTON, ZOOM_SENSITIVITY
from .painting import ViewportMapper, draw_shape
...
def set_shapes_hovered(shapes, cursor, selection_rect=None):
    ...
    selection_shapes_hovered = [
        ...
        if s.rect.contains(cursor) or s.rect.intersects(selection_rect)
    ]
...
class PickerView(QtWidgets.QWidget):
    ...
    def mouseReleaseEvent(self, event):
        ...
        interact = (
            self.clicked_shape
            and self.clicked_shape is detect_hovered_shape(self.shapes, cursor)
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.mode_manager.mode == ModeManager.DRAGGING:
            point1 = self.viewportmapper.to_units_coords(self.mode_manager.anchor)
...
class ModeManager:
    ...
    def update(self, event, pressed=False, has_shape_hovered=False, dragging=False):
