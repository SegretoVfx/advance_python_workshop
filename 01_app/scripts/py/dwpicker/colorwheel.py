...
from .geometry import get_relative_point, get_point_on_line, get_absolute_angle_c
...
class ColorWheel(QtWidgets.QWidget):
    ...
    def paint(self, painter):
        ...
        painter.drawRoundedRect(
            6, 6, (self.width() - 12), (self.height() - 12), self.width(), self.height()
        )
        ...
        self._horizontal_gradient.setColorAt(1.0, self._get_current_wheel_color())
        ...
        painter.drawLine(get_point_on_line(angle, 37), get_point_on_line(angle, 46))
    ...
    def set_current_color(self, color):
        ...
        x = (
            ((sorted([r, g, b], reverse=True)[0] - sorted([r, g, b])[0]) / 255.0)
            * self._rect.width()
        ) + self._rect.left()

        y = (
            ((255 - (sorted([r, g, b], reverse=True)[0])) / 255.0) * self._rect.height()
        ) + self._rect.top()
...
def degree_to_color(degree):
    ...
    if degree >= 0.0 and degree <= 0.66:
        if degree <= 0.16:
            g = round(255.0 * (degree / 0.16))
        elif degree < 0.5:
            g = 255
        if degree >= 0.5:
            factor = degree - 0.5
            g = 255 - round(255.0 * (factor / 0.16))
    else:
        g = 0
    g = min(g, 255.0)
    g = max(g, 0)
    ...
    if degree >= 0.33 and degree <= 1.0:
        if degree <= 0.5:
            factor = degree - 0.33
            b = round(255 * (factor / 0.16))
        elif degree < 0.83:
            b = 255.0
        if degree >= 0.83 and degree <= 1.0:
            factor = degree - 0.83
            b = 255.0 - round(255.0 * (factor / 0.16))
    else:
        b = 0
    b = min(b, 255)
    b = max(b, 0)
