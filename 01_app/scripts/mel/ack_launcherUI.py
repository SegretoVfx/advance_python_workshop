# ------------------------------------------------------------
# --- ackTools popup ---
# Description   = Creates a popup with buttons to launch ackTools scripts
#
# Date   = 2021 - 03
# Update = 2024 - 01 - 29
# Author = Juls
...
class AckToolsLauncher(object):
    def __init__(self):
        # Global Variables
        # self.UI()
        self.ack_toolbox_dir = f"{path.dirname(__file__)}/ack_toolbox_menu"
    ...
    def launch_mel_script(self, mel_name, option, *args):
        """Read the mel script from it's mel file and launch it"""
        file = open(f"{self.ack_toolbox_dir}/{mel_name}.mel")
        content = file.read()  # .replace('\n',' ')
        file.close()
        # print(content)
        # Load the script
        # Call the script with some variation if any
        if mel_name == "ackSetup":
            mel.eval(f"{content}")
            mel.eval(f"{mel_name} {option};")
        elif mel_name == "ackToggleCams":
            if option == "setup":
                mel.eval(f"{content}; {mel_name}_{option};")
            else:
                mel.eval(f"{content}; {mel_name};")
        elif (
            mel_name == "ackMoveKeys"
            or mel_name == "ackConvergeBuffer"
            or mel_name == "ackSpreadSqueezeTiming"
            or mel_name == "ackPushPull"
        ):
            mel.eval(f"{content}")
            mel.eval(f"{mel_name} {option};")
        else:
    ...
    def viewport_option_ui(self, *args):
        ...
        cmds.rowLayout(numberOfColumns=1)
        cmds.text(label="Toggle view on current viewport")
        cmds.setParent("..")
...
class AckGraphToolsLauncher(object):
    def __init__(self):
        # Global Variables
        # self.UI()
        self.ack_toolbox_dir = "{0}/ack_toolbox_menu".format(path.dirname(__file__))
        self.viewport_option_ui()
    ...
    def launch_mel_script(self, mel_name, option, *args):
        """Read the mel script from it's mel file and launch it"""
        file = open("{0}/{1}.mel".format(self.ack_toolbox_dir, mel_name))
        content = file.read()  # .replace('\n',' ')
        file.close()
        # print(content)
        # Load the script
