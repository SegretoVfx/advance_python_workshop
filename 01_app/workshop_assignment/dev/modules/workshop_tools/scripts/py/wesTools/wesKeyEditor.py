...
def checkChannelBoxSelection(object_sel):
    if debug_chch_mode:
        print(
            "##############Debug Check Channel Box Selection ########################"
        )
    ...
    if len(channel_box_selections) != 0:
        ...
        for chb in channel_box_selections:
            ...
            attr_name = buffer[1]
            if debug_chch_mode:
                print("buffer for splitting:  " + str(buffer))
            if debug_chch_mode:
                print("control_name:  " + str(control_name))
            if debug_chch_mode:
            ...
            if debug_chch_mode:
                print("attr_long_name:  " + str(attr_long_name))
            ...
            if debug_chch_mode:
                print("curve_name:  " + str(curve_name))
        ...
        if debug_chch_mode:
            print("posssible curves:  " + str(possible_curves))
        ...
        if debug_chch_mode:
            print("real_curve:  " + str(real_curve))
...
def editKeys(power, operation):
    ...
    for sel in selected:
        ...
        if debug_edit_mod:
            print("graph editor selection =  " + str(checkGraphEditorSelection(sel)))

        # Adding a checker to see if graph editor is selected
        if not checkGraphEditorSelection(sel):
            print(
                "Nothing is selected in graph editor cause :: "
                + str(checkGraphEditorSelection(sel))
            )
            ...
            add_this = checkGraphEditorSelection(sel) or createCurrentFrameSelection(
                sel
            )
        else:
    ...
    curveModifier(
        power, operation, full_list_curves
    )  ##########################################
...
def curveModifier(power, operation, sent_list_curves):
    ...
    if debug_curve_mod:
        print("Curve list argument:  " + str(sent_list_curves))

    for curve in sent_list_curves:
        if debug_curve_mod:
            print("Current Curve To Edit:  " + str(curve))
        ...
        if debug_curve_mod:
            print("selected keyframe index:  " + str(sel_keyf_indx))

        keyf_indx = cmds.keyframe(curve, query=True, indexValue=True)
        if debug_curve_mod:
            print("keyframe index:  " + str(keyf_indx))
        ...
        if not sel_keyf_indx == None:
            ...
            if before_check in keyf_indx:
                ...
                if debug_curve_mod:
                    print("before Value:   " + str(before) + "  is Previous Keyframe")
            else:
                before = keys[0]
                if debug_curve_mod:
                    print("before Value:   " + str(before) + "  is Same Keyframe")
            ...
            if after_check in keyf_indx:
                ...
                if debug_curve_mod:
                    print("after Value:   " + str(after) + "  is Next Keyframe")
            else:
                after = keys[-1]
                if debug_curve_mod:
                    print("after Value:   " + str(after) + "  is Same Keyframe")
            ...
            if debug_curve_mod:
                print("This is the keys:  " + str(keys))
            for position in range(len(keys)):
                ...
                if operation == "shiftKeys":
                    ...
                    if prev_key[1] == (keys[position][1] + power):
                        ...
                        cmds.headsUpMessage(
                            "There is a Key that couldn't shift to the left", time=3
                        )
                        ...
                        print("I dont move this key:  " + str(curve))
                        cmds.headsUpMessage(
                            "There is a Key that couldn't shift to the right", time=3
                        )
                        ...
                        cmds.keyframe(
                            curve,
                            index=(keys[position][0], keys[position][0]),
                            edit=True,
                            timeChange=power,
                            relative=True,
                        )
                        if debug_curve_mod:
                            print("current Keys Value:  " + str(keys[position]))

                ######## Increase/Decrease Tangents ######
                #   Checking the out angle
                elif operation == "steepenTangent":
                    if debug_curve_mod:
                        print("=========STEEPEN TANGENTS!============")

                    tang_angle = cmds.keyTangent(
                        curve, q=True, index=(keys[position][0],), oa=True
                    )[0]
                    if tang_angle < 0:
                        add_val = -power
                    elif tang_angle > 0:
                    ...
                    key_time = cmds.keyframe(
                        curve, q=True, index=(keys[position][0],), tc=True
                    )[0]

                    if debug_curve_mod:
                        print("tang_angle:  " + str(tang_angle))
                    if debug_curve_mod:
                        print("keys position index:  " + str(keys[position][0]))
                    if debug_curve_mod:
                        print("add_val:  " + str(add_val))
                    if debug_curve_mod:
                        print("new_angle:  " + str(new_angle))
                    if debug_curve_mod:
                        print("key_time:  " + str(key_time))
                    ...
                    if operation == "flatxagerate":
                        ...
                        ratio_time = (keys[position][1] - before[1]) / (
                            after[1] - before[1]
                        )
                        ...
                        if power < 1:
                            ...
                            if old_value > target_value:
                                new_value = old_value - (
                                    abs(old_value - target_value) * power
                                )
...
def UI(parentWindow=None, user_width=180, user_height=17, frameClosed=False):
    ...
    cmds.button(
        l="-",
        command=lambda x: scaleKeysby("flatten"),
        width=user_width * 0.4,
        height=user_height,
        bgc=[0.701, 0.286, 0.521],
        annotation="Flatten Your Curves!",
    )
    cmds.button(
        l="+",
        command=lambda x: scaleKeysby("exaggerate"),
        width=user_width * 0.4,
        height=user_height,
        bgc=[0.286, 0.701, 0.467],
        annotation="Exaggerate Your Curves!",
    )
    cmds.intField(
        "percentageScale",
        minValue=0,
        maxValue=100,
        width=user_width * 0.15,
        height=user_height,
        value=20,
        annotation="The Percentage of which the curve will be affected",
    )
    cmds.text(
        "%",
        width=user_width * 0.05,
        height=user_height,
        annotation="The Percentage of which the curve will be affected",
    )
    cmds.setParent("..")
    ...
    cmds.button(
        l="<<",
        command=lambda x: scaleKeysby("cushionLeft"),
        width=user_width * 0.4,
        height=user_height,
        bgc=[0.651, 0.286, 0.749],
        annotation="Favor to the Left! (Ctrl + Click = Match Left)",
    )
    cmds.button(
        l=">>",
        command=lambda x: scaleKeysby("cushionRight"),
        width=user_width * 0.4,
        height=user_height,
        bgc=[0.286, 0.615, 0.615],
        annotation="Favor to the Right! (Ctrl + Click = Match Right)",
    )
    cmds.text(
        "favor", width=user_width * 0.2, height=user_height, font="plainLabelFont"
    )
    cmds.setParent("..")
    ...
    cmds.button(
        l="<<",
        command=lambda x: editKeys(-2, "shiftKeys"),
        width=user_width * 0.170,
        height=user_height,
        bgc=[1, 0.4, 0.2],
    )
    cmds.button(
        l="<",
        command=lambda x: editKeys(-1, "shiftKeys"),
        width=user_width * 0.214,
        height=user_height,
        bgc=[1, 0.7, 0.23],
    )
    cmds.button(
    ...
    cmds.button(
        l="#",
        command=lambda x: shiftKeysBy(),
        width=user_width * 0.107,
        height=user_height,
        bgc=[0.95, 0.85, 0.58],
        annotation="by # value you input",
    )
    cmds.intField(
        "entered_shift_value", value=10, width=user_width * 0.125, height=user_height
    )
    cmds.setParent("..")
    ...
    cmds.checkBox(
        ...
        editable=True,
        value=True,
        annotation="If you want the tangents to be smoothed out after the keys have moved, check this on.",
    )
    cmds.setParent("..")
